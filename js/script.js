...
mailbox.addEventListener('click', () => {
  ...
  if (!isOpen && !letterDelivered) {
    ...
    isOpen = true;
    notification.textContent = "📥 یه نامه؟ از توی صندوق برش دار ببینیم توش چیه";
    notifySound.currentTime = 0;
...
itemSlots.forEach(slot => {
  ...
  longTextModal.style.display = 'flex';
        longTextContent.innerHTML = `
          <p style="font-size: 1.4em; font-weight: bold; text-align: center;">101 دلیل که عاشقتم</p>
...
<p>50. خیلی بدن پرستیدنی داری/p>
<p>51. خیلی سکسی ای و من جذبت میشم</p>
<p>52. بدنت تحریکم میکنه</p>
<p>53. موهات جذابه/p>
...
<p>71. لبات مثل جام شراب من میمونه</p>
...
<p>100. زندگی با تو قشنگه</p>
  ...
  <p style="margin-top: 30px; font-style: italic;">پ.ن: یه سوپرایز برات توی پاکت گذاشتم، ازش خوب مراقبت کن.</p>
`;
if (!seedTaken) {
...
modal.addEventListener('click', () => {
  if (letterStage === 1) {
    ...
    caption.textContent = "با تمام وجودم میپرستمت.";
    ...
    img.addEventListener('mousedown', (e) => {
      ...
      function onMouseUp(e) {
        if (isDragging) {
          ...
          } else if (img.src.includes('water.png') && holeFilled && !holeWatered) {
            const hole = document.getElementById('hole-img');
            if (hole) {
              const holeRect = hole.getBoundingClientRect();
              const dropX = e.clientX;
              ...
              if (dropX >= holeRect.left && dropX <= holeRect.right &&
                  dropY >= holeRect.top && dropY <= holeRect.bottom) {
                hole.src = 'images/hole-wet.png';
                holeWatered = true;
                const starsSound = new Audio('sounds/stars.mp3');
                starsSound.play().catch(err => console.warn("⛔ پخش صدای ستاره شکست خورد:", err));
                notification.textContent = "آفرین به دونت آب  دادی، حالا صبر کن تا رشد کنه";
                notifySound.currentTime = 0;
                notifySound.play().catch(err => {});

                // بعد از 5 ثانیه، تصویر به hole-seed.png تغییر کنه
                setTimeout(() => {
                  hole.src = 'images/hole-seed.png';
                  const starsSound = new Audio('sounds/stars.mp3');
                  starsSound.play().catch(err => console.warn("⛔ پخش صدای ستاره شکست خورد:", err));
                  notification.textContent = "دونه رشد کرد ولی... چرا حس عجیبی داره؟";
                  notifySound.currentTime = 0;
                  notifySound.play().catch(err => console.warn("⛔ پخش صدای نوتیف شکست خورد:", err));

                  // بعد از 5 ثانیه دیگه، صدای earth.mp3 پخش بشه و صفحه بلرزه به مدت 5 ثانیه، سپس محو بشه
                  setTimeout(() => {
                    const earthSound = new Audio('sounds/earth.mp3');
                    earthSound.play().catch(err => console.warn("⛔ پخش صدای زمین شکست خورد:", err));

                    // لرزیدن صفحه به مدت 5 ثانیه
                    document.body.classList.add('shake');
                    setTimeout(() => {
                      document.body.classList.remove('shake');
                      
                      // بعد از اتمام لرزیدن، محو شدن صفحه
                      document.body.style.transition = 'opacity 2s';
                      document.body.style.opacity = 0;

                      // بعد از محو شدن، پروژه جدید رو لود کن
                      setTimeout(() => {
                        // پاک کردن محتوای فعلی صفحه
                        document.body.innerHTML = '';

                        // اضافه کردن iframe برای پروژه جدید
                        const iframe = document.createElement('iframe');
                        iframe.src = 'new_project/index.html'; // مسیر اصلاح‌شده برای لوکال
                        iframe.style.width = '100vw';
                        iframe.style.height = '100vh';
                        iframe.style.border = 'none';
                        document.body.appendChild(iframe);

                        // برگردوندن opacity به 1 برای نمایش پروژه جدید
                        document.body.style.opacity = 1;
                      }, 2000); // زمان محو شدن (2 ثانیه)
                    }, 5000); // مدت زمان لرزیدن
                  }, 5000); // زمان قبل از پخش صدا و لرزیدن
                }, 5000); // زمان قبل از تغییر تصویر به hole-seed.png
              }
            }
          }
